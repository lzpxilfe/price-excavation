      ...
      <div className="step-grid map-step">
        <section className="card form-card">
          ...
          <ConsentRow
            ...
            description="켜면 주소는 VWorld에, 지도 타일은 기본 지도 제공자에 조회합니다. 응답은 화면에만 표시하며 저장·내보내기하지 않습니다."
          />
        </section>
        <section className="card map-card" aria-label="현장 위치 미리보기">
          ...
          <div className="map-caption"><span className="status-dot ok" /> {project.location.externalLookup ? (project.location.parcelReferenceGeoJson ? "기본 지도와 VWorld 연속지적도 참고경계를 표시했습니다." : "기본 지도에 입력 좌표를 표시했습니다.") : "외부 지도 조회 동의를 켜면 위치 미리보기를 표시합니다."}<small>참고경계는 지적측량성과가 아니며, 체적 계산은 별도 측량 GeoJSON으로 확정하세요.</small></div>
        </section>
      </div>
    );
  }
